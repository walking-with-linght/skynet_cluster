...
int ikcp_input(ikcpcb *kcp, const char *data, long size)
{
	...
	while (1) {
		...
		IUINT8 cmd, frg;
		IKCPSEG *seg;
		//printf("2------ikcp_input size = %ld IKCP_OVERHEAD = %d\n", size, (int)IKCP_OVERHEAD);
		if (size < (int)IKCP_OVERHEAD) break;

		data = ikcp_decode32u(data, &conv);
		//printf("3------ikcp_input conv = %u kcp->conv = %u\n", conv, kcp->conv);
		...
		if (cmd == IKCP_CMD_ACK) {
			...
			//printf("4------ikcp_input conv = %u kcp->conv = %u\n", conv, kcp->conv);
			if (_itimediff(sn, kcp->rcv_nxt + kcp->rcv_wnd) < 0) {
				ikcp_ack_push(kcp, sn, ts);
				//printf("5------ikcp_input sn = %u kcp->rcv_nxt = %u\n", sn, kcp->rcv_nxt);
				if (_itimediff(sn, kcp->rcv_nxt) >= 0) {
					...
					seg->una = una;
					seg->len = len;
					//printf("6------ikcp_input conv = %u kcp->conv = %u len = %u\n", conv, kcp->conv, len);
					if (len > 0) {
						//printf("7------ikcp_input conv = %u kcp->conv = %u\n", conv, kcp->conv);
