...
static int write_sz(lua_State *L){
...
static int read_n(lua_State *L){
	...
	while(n > 0){
		...
		if(r < 0){
			...
			luaL_error(L, "read fd error: %s", strerror(errno));
		}
		n -= r;
	}
	s[sz] = 0;
