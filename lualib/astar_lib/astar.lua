...
local BinaryHeap = require('astar_lib.binaryheap')
...
function M:init(map)
    assert(map.get_neighbors and map.get_cost and map.estimate_cost,
...
function M:find(start, goal, userdata)
    ...
    local add_neighbor_fn = function(neighbor, cost)
        if not closedset[neighbor] then
            if not cost then
                cost = map:get_cost(current, neighbor, userdata)
            end
            ...
            if not openmap[neighbor] or tentative_g_score < g_score[neighbor] then
                ...
                h_score[neighbor] = h_score[neighbor] or
                                        map:estimate_cost(neighbor, goal,
                                                          userdata)
