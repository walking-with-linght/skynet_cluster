...
M.binaryHeap = function(swap, erase, lt)
    ...
    function heap:bubbleUp(pos)
        ...
        while pos > 1 do
            ...
            if not lt(values[pos], values[parent]) then break end
...
pop =
    function(self) if self.values[1] ~= nil then return remove(self, 1) end end

local peek
--- Returns the element at the top of the heap, without removing it.
-- @function heap:peek
-- @return value at the top, or `nil` if there is none
peek = function(self) return self.values[1] end
...
local function erase(heap, pos) heap.values[pos] = nil end
...
M.maxHeap = function(gt)
    if not gt then gt = function(a, b) return (a > b) end end
    return plainHeap(gt)
end
...
function removeU(self, payload)
    ...
    if pos ~= nil then return remove(self, pos), payload end
end
...
-- @return payload, value, or `nil` if there is none
peekU = function(self) return self.payloads[1], self.values[1] end
...
-- end
peekValueU = function(self) return self.values[1] end
...
-- @function heap:size
-- @return number of elements
sizeU = function(self) return #self.values end
