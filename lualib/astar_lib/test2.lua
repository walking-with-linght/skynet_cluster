...
local map = {}
local size = 50
local function table_new(len, default)
    local tab = {}
    for i = 1, len do tab[i] = {} end
    return tab
end
local cached_nodes = table_new(size * size, 0)
...
for x = 1, size do
    for y = 1, size do
        ...
        -- print(idx, node.x, node.y, node.s)
...
local function test(name, sx, sy, tx, ty, print_path)
    ...
    for i = 1, 3 do
        ...
        local tnodes = 0
        for n, v in pairs(g_score) do tnodes = tnodes + 1 end

        if path then
            print(string.format(
                      'cost: %9.4fms, test valid nodes: %i, path nodes: %s',
                      cost, tnodes, #path))
            if print_path then
                local r = {}
            ...
            print(string.format(
                      "cost: %.4fms, tested valid nodes: %i, Not found path",
                      cost, tnodes))
        end
    end
end
