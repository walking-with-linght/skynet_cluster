...
--获取相对路径
function M.getRelativePath(basePath, fullPath)
    -- 参数检查
    if not basePath or not fullPath or type(basePath) ~= "string" or type(fullPath) ~= "string" then
        return nil
    end
    
    -- 确保路径使用统一的路径分隔符（将反斜杠转换为正斜杠）
    basePath = basePath:gsub("\\", "/")
    fullPath = fullPath:gsub("\\", "/")
    
    -- 移除末尾的路径分隔符（如果有）
    basePath = basePath:gsub("/+$", "")  -- 移除末尾的一个或多个斜杠
    fullPath = fullPath:gsub("/+$", "")  -- 移除末尾的一个或多个斜杠
    
    -- 检查 fullPath 是否以 basePath 开头
    if fullPath:sub(1, #basePath) == basePath then
        -- 获取相对路径部分
        local remainingPath = fullPath:sub(#basePath + 1)
        ...
        -- 确保相对路径以斜杠开头
        if remainingPath:sub(1, 1) ~= "/" then
            remainingPath = "/" .. remainingPath
        end
        
        return remainingPath
    else
        -- 如果不以 basePath 开头，返回 nil
        return nil
    end
end

--获取文件路径
function M.getDirectoryPath(filePath)
    -- 参数检查
    ...
    -- 查找最后一个路径分隔符的位置
    local lastSlashIndex = filePath:find("/[^/]*$")
    
    if lastSlashIndex then
        -- 提取目录路径（包含最后一个斜杠）
        local directoryPath = filePath:sub(1, lastSlashIndex)
        return directoryPath
    else
        -- 如果没有找到路径分隔符，说明是当前目录的文件
        return "./"
    end
end
