...
function ws.new(id, header, handler, conf,addr)
    local conf = conf or {}
    ...
    self.handler.on_open(self,addr)
    return setmetatable(self, ws_mt)
end


function ws_send_frame(id,fin, opcode, data,mask_outgoing)
    ...
    -- debug: 打印数字而不是 frame 本身
    local header_len = #frame
    local total_len = header_len + #data
    -- print(string.format("发送字节长度 total=%d header=%d payload=%d mask_outgoing=%s", total_len, header_len, #data, tostring(mask_outgoing)))
...
function ws:send_frame(fin, opcode, data)   
    return ws_send_frame(self.id,fin,opcode, data,self.mask_outgoing) 
end
...
function ws_send_binary(id,data)
    return ws_send_frame(id,true,0x2, data)
end
...
function ws:recv_frame()
    ...
    if reserved_bits then
        ...
        skynet.error("Reserved_bits show using undefined extensions")
    ...
    -- debug: print frame header info
    -- skynet.error(string.format("[ws] recv header=0x%02X final=%s opcode=0x%X control=%s mask=%s payloadlen(raw)=%d",
    --     header,
    --     tostring(final_frame),
    --     frame_opcode,
    --     tostring(frame_opcode_is_control),
    --     tostring(mask_frame),
    --     payloadlen))
